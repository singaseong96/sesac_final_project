...
투자 분석 파이프라인
────────────────────────────────────────────────────────
Step 1. 뉴스 감정 분석  (OpenAI API → 6개 레이블)
Step 2. 투자 판단       (OpenAI API → 시스템/유저 프롬프트)
...
# OpenAI API 호출
# ─────────────────────────────────────────────────────────
def call_gpt(system: str, user: str, max_tokens: int = MAX_TOKENS) -> str:
    headers = {
...
# ✅ [수정] Cloudflare·보안 차단 페이지 키워드 목록
BLOCKED_KEYWORDS = [
    "security service",
    "banned you",
    "cookie tech",
...
# ✅ [수정] 단건 분석 전용 프롬프트 (날짜 구분 지시 제거, 레이블만 출력)
...
def analyze_sentiment(title: str, content: str) -> str:
    ...
    try:
        # ✅ 추론 모델은 reasoning 토큰을 내부 소비 → 출력 토큰 여유 필요
        label = call_gpt(SENTIMENT_SYSTEM, user, max_tokens=300)

        # ✅ 마크다운 기호·구두점 제거 후 유효 레이블 탐색
        valid = {"Positive", "Negative", "Neutral", "Growth", "Risk", "Mixed"}
        cleaned = re.sub(r"[^a-zA-Z\s]", "", label)   # **, . 등 제거
...
def main(SYMBOL):
    ...
    judgment = call_gpt(INVESTMENT_SYSTEM, user_prompt, max_tokens=1500)
    print("\n── 투자 판단 결과 ──")
    print(judgment)
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{SYMBOL}_analysis_GPT.json")
